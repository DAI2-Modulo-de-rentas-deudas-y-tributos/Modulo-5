...
- La identidad sale de la sesión DEMO persistida (`X-Demo-Session` → `demo_auth_session`). Un contribuyente no puede consultar recursos de otro (`403 FORBIDDEN_OWNERSHIP`). Las cabeceras `X-Dev-*` no otorgan privilegios.
