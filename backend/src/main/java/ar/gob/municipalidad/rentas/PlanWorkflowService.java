...
class PlanWorkflowService {
    ...
    PlanWorkflowService(PaymentPlanConfigurationRepository configurations, PaymentPlanRequestRepository requests,
        ...
        this.debts=debts; this.taxConfigurations=taxConfigurations; this.identity=identity; this.audit=audit; this.paymentEvents=paymentEvents;
    }
    ...
    @Transactional PaymentPlanRequest request(ApiDtos.CreatePaymentPlanRequest r) {
        ...
        audit.record("PaymentPlanRequest",request.id,"PAYMENT_PLAN_REQUESTED",request); return request;
    }
    ...
    @Transactional PaymentPlanRequest grant(Long id,ApiDtos.GrantPaymentPlanRequest body) {
        PaymentPlanRequest r=getRequest(id); requireStatus(r,PaymentPlanRequestStatus.PENDING);
        ...
        audit.record("PaymentPlanRequest",r.id,"PAYMENT_PLAN_GRANTED",r);return r;
    }
    ...
    Page<PaymentPlan> defaulted(Pageable pageable){List<PaymentPlan> all=plans.findAll().stream().filter(this::isDefaulted).toList();int start=(int)Math.min(pageable.getOffset(),all.size());int end=Math.min(start+pageable.getPageSize(),all.size());return new PageImpl<>(all.subList(start,end),pageable,all.size());}
    ...
    private PaymentPlanRequest reject(PaymentPlanRequest r,String reason,String action){r.status=PaymentPlanRequestStatus.REJECTED;r.resolutionReason=reason;r.resolvedBy=identity.get().userId();r.resolvedAt=OffsetDateTime.now();audit.record("PaymentPlanRequest",r.id,action,r);return r;}
