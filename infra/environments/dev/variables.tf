...
variable "public_subnet_cidrs" {
  description = "CIDR de las subredes publicas DEV."
  type        = list(string)
...
variable "backend_task_cpu" {
  description = "CPU de la tarea Fargate DEV."
  type        = number
  default     = 256
}

variable "backend_task_memory" {
  description = "Memoria MiB de la tarea Fargate DEV."
  type        = number
  default     = 512
}

variable "backend_spring_profiles_active" {
  description = "Perfiles activos del backend en DEV."
  type        = string
  default     = "dev"
}

variable "backend_environment_variables" {
  description = "Variables no sensibles del backend en DEV."
  type        = map(string)
  default = {
...
variable "backend_secret_variables" {
  description = "Variables sensibles del backend referenciadas por ARN de Secrets Manager en DEV."
  type        = map(string)
  default     = {}
}

variable "frontend_environment_variables" {
  description = "Variables publicas incorporadas al build del frontend en DEV."
  type        = map(string)
  default = {
    VITE_AUTH_MODE = "mock"
...
variable "database_instance_class" {
  description = "Clase RDS de bajo costo para DEV."
  type        = string
...
variable "database_backup_retention_days" {
  description = "Retencion de backups automatizados en DEV."
  type        = number
  default     = 1
}
...
variable "database_skip_final_snapshot" {
  description = "Omite el snapshot final al destruir DEV."
  type        = bool
  default     = true
}
